...
import { emptyData, genId, orderCodeFromCounter, todayStr, daysBetween, clamp0, applyAppIcon, orderBalance, migrateData, adjustStock, updateStockPrice, pushTrash, pushStockMovement } from "./utils.js";
...
export default function App() {
  ...
  function exportExcel() {
    ...
    data.gameAccounts.forEach(a => (a.stock || []).forEach(s => stockRows.push({
      ...
      คงเหลือ: s.quantity, แจ้งเตือนเมื่อเหลือ: s.lowStockThreshold ?? 2, ราคาต่อตัว: s.price || 0,
    })));
  ...
  return (
    <div className="pgs-root">
      ...
      <div className="pgs-scroll">
        {tab === "dashboard" && <Dashboard data={data} stats={stats} custName={custName} accName={accName} goTab={navigateTab} openDetail={(d) => setDetail(d)} />}
        {tab === "orders" && (
          <OrdersTab
