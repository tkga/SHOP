...
export default function OrderModal({ data, mode, item, onClose, onSave, onCancel, onRestore, onDelete, onReceipt }) {
  ...
  // รายชื่อ Pokémon สำหรับ datalist — จำกัดเฉพาะที่มีในสต๊อกของไอดีต้นทางนี้เท่านั้น (ไม่ดึงจากไอดีอื่นหรือออเดอร์เก่า)
  const stockNamesForAccount = Array.from(new Set(stockOptions.map(s => (s.name || "").trim()).filter(Boolean)))
  ...
  useEffect(() => {
    if (form.stockItemId) {
      ...
      if (s) { setStockNameSel((s.name || "").trim().toLowerCase()); return; }
    }
    setStockNameSel("");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // ถ้าเป็น "จ้างฟามทั่วไป" ต้องกรอกรายการฟามมาก่อน (ไม่งั้นจะไม่รู้ว่าจ้างฟามอะไร)
  const canSubmit = !!form.customerId
    && (!isSell || matchedVariantKeys.length > 0)
        ...
        <>
          ...
          {stockOptions.length > 0 && (
            <div className="pgs-field">
              ...
              <select
                className="pgs-select" value={stockNameSel}
                onChange={e => {
                  const key = e.target.value;
                  setStockNameSel(key);
                  if (!key) { set("stockItemId", null); return; }
                  const group = stockGroups[key];
                  if (group && group.items.length === 1) pickStock(group.items[0].id);
                  else set("stockItemId", null); // มีหลายประเภท — รอเลือก variant ด้านล่างก่อน
                }}
              >
                <option value="">- กรอกเอง (ไม่ตัดสต๊อก) -</option>
                {stockGroupList.map(([key, g]) => {
                  const totalQty = g.items.reduce((s, it) => s + (Number(it.quantity) || 0), 0);
                  return <option key={key} value={key}>{g.name} · เหลือ {totalQty}</option>;
                })}
              </select>
              {stockNameSel && stockGroups[stockNameSel]?.items.length > 1 && (
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <div className="pgs-field" style={{ flex: 2 }}>
              ...
              <input className="pgs-input" list="pgs-pokemon-names" value={form.pokemonName} onChange={e => set("pokemonName", e.target.value)} placeholder="เช่น Rayquaza" disabled={!!form.stockItemId} />
              <datalist id="pgs-pokemon-names">
                {stockNamesForAccount.map(n => <option key={n} value={n} />)}
              </datalist>
              {stockOptions.length === 0 && (
