...
export default function OrderModal({ data, mode, item, onClose, onSave, onCancel, onRestore, onDelete, onReceipt }) {
  ...
  // ให้เหลือแค่ประเภทที่มีของจริง — ถ้าไม่เจอชื่อนี้ในสต๊อกเลย จะไม่มีตัวเลือกให้เลือกเลย (ต้องเพิ่มสต๊อกก่อน)
  const trimmedName = (form.pokemonName || "").trim();
  const matchedVariantKeys = variantKeysForPokemonName(data, trimmedName);
  ...
  // กันบันทึกออเดอร์ที่ข้อมูลไม่ครบ: ต้องมีลูกค้า และถ้าเป็นออเดอร์ "ขาย" ชื่อ Pokémon ต้องมีของในสต๊อกจริง (เลือกประเภทได้อย่างน้อย 1 ชนิด)
  const canSubmit = !!form.customerId && (!isSell || matchedVariantKeys.length > 0);
        ...
        <>
          ...
          <div className="pgs-field">
            ...
            {form.stockItemId ? (
              ...
              <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 6 }}>
                {trimmedName} มีในสต๊อก {matchedVariantKeys.length} ประเภท จึงเลือกได้เฉพาะประเภทเหล่านี้
              </div>
            ) : trimmedName ? (
              <div style={{ fontSize: 11, color: "var(--red)", marginTop: 6 }}>
                ไม่พบ "{trimmedName}" ในสต๊อก — ต้องเพิ่มสต๊อกของ Pokémon นี้ก่อน ถึงจะเลือกประเภทและสร้างออเดอร์ได้
              </div>
            ) : (
              <div style={{ fontSize: 11, color: "var(--red)", marginTop: 6 }}>
