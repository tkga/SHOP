...
import { fmtMoney, fmtDate, orderBalance, clamp0 } from "../utils.js";
...
export default function OrdersTab({
  ...
  const filtered = data.orders.filter(o => {
    ...
    if (tradeFilter !== "all") {
      if (o.type !== "sell_pokemon" || o.tradeStatus !== tradeFilter) return false;
    }
    if (hireFilter !== "all") {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 6, overflowX: "auto", marginBottom: 8, paddingBottom: 2 }}>
        ...
        <button className={"pgs-chip" + (filter === "sell_pokemon" ? " active" : "")} onClick={() => setFilter("sell_pokemon")}>{ORDER_TYPES.sell_pokemon?.emoji} เทรด</button>
        <button className={"pgs-chip" + (filter === "hire_boss" ? " active" : "")} onClick={() => setFilter("hire_boss")}>{ORDER_TYPES.hire_boss?.emoji} ตีบอส</button>
        <button className={"pgs-chip" + (filter === "hire_invite" ? " active" : "")} onClick={() => setFilter("hire_invite")}>{ORDER_TYPES.hire_invite?.emoji} เชิญตี</button>
      ...
      {showAdv && (
        <div className="pgs-card" style={{ marginBottom: 12 }}>
          ...
          {filter !== "hire_boss" && filter !== "hire_invite" && (
            <div className="pgs-field" style={{ marginBottom: 10 }}>
              <label className="pgs-label">ระบบเทรด</label>
          ...
          {filter !== "sell_pokemon" && (
            <div className="pgs-field" style={{ marginBottom: 10 }}>
              <label className="pgs-label">ตีบอส/เชิญตี</label>
              <div style={{ display: "flex", gap: 6 }}>
      ...
      {filtered.length === 0 ? <EmptyState text="ไม่พบออเดอร์" /> : filtered.map(o => {
        ...
        return (
          <div key={o.id} className="pgs-card" style={{ marginBottom: 8, opacity: o.cancelled ? 0.7 : 1 }} onClick={() => openEdit(o)}>
            ...
            {!o.cancelled && isHire && o.hireStatus !== "done" && hireFull && (
              <button
