        ...
        <div style={{ position: "relative", zIndex: 1, padding: 22 }}>
          ...
          <div style={{ marginBottom: 14 }}>
            <div className="pgs-row" style={{ marginBottom: 4 }}>
              <span style={{ fontWeight: 700, fontSize: 15, color: HERO.text }}>{r.customerName}</span>
              <span style={{ fontSize: 11, fontWeight: 700, color: HERO.accent }}>ครั้งที่ {r.orderNoForCustomer}</span>
            </div>
            <div style={{ fontSize: 12, color: HERO.textMuted }}>
              {r.serviceEmoji} {r.serviceLabel}{r.customerGameId ? ` · ไอดี ${r.customerGameId}` : ""}
            </div>
            {r.periodStr && <div style={{ fontSize: 12, color: HERO.textMuted, marginTop: 2 }}>🗓️ {r.periodStr}</div>}
          </div>
